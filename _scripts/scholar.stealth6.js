...
async function checkCaptcha(driver) {
    try {
        // Try to find an element or text typical of the CAPTCHA page
        let captchaElems = await driver.findElements(By.xpath("//*[contains(text(),'unusual traffic') or contains(@id,'captcha')]"));
        if (captchaElems.length > 0) {
            console.log("CAPTCHA detected! Please solve it manually. Waiting 1 minute...");
            await sleep(60000); // wait 60s
            return true;
        }
    } catch (err) {
        console.log("Error while checking CAPTCHA:", err);
    }
    return false;
}
