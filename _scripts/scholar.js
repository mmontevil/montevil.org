...
async function isCaptchaPresent(driver) {
  try {
    ...
    if ((await driver.findElements(By.css("input#captcha, input[name*='captcha'], input[id*='captcha']"))).length)
      return true;
    if ((await driver.findElements(By.css("[class*='captcha'], [id*='captcha']"))).length)
      return true;
    if ((await driver.findElements(By.css("iframe[src*='recaptcha'], iframe[src*='google.com/recaptcha']"))).length)
      return true;
...
async function safeGet(driver, url, retries = 3) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (!(await waitForCaptchaSolve(driver))) throw new Error('CAPTCHA timeout');
...
(async () => {
  ...
  const updatedList = [];
  const listmypub = await driver.findElements(By.xpath("//tr[@class='gsc_a_tr']"));
  for (const pubElement of listmypub) {
    const title0 = await pubElement.findElement(By.xpath(".//a[@class='gsc_a_at']"));
    const titleText = await title0.getText();
    const numberCiteEls = await pubElement.findElements(By.xpath(".//td[@class='gsc_a_c']/a[not(@data-eid)]"));
    if (numberCiteEls.length > 0) {
      const numberCite = numberCiteEls[0];
      const numberCiteText = await numberCite.getText();
      const linkciting = numberCiteText.length > 0 ? await numberCite.getAttribute('href') : '';
      updatedList.push({ title: titleText, citingLink: linkciting, citenumber: numberCiteText || 0 });
    }
  }
