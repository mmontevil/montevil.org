...
async function humanScroll(driver) {
    const scrollTimes = Math.floor(Math.random() * 5) + 3; // 3 à 7 scrolls
    for (let i = 0; i < scrollTimes; i++) {
        const scrollStep = Math.floor(Math.random() * 400) + 200; // 200 à 600 px
        await driver.executeScript(`window.scrollBy(0, ${scrollStep});`);
        await adsleep(800, 1800);
    }
}
...
(async function example() {
    ...
    while (temp) {
        ...
        if (belows.length > 0) {
            ...
            await adsleep(5000, 10000); // délai plus long pour pages multiples
        } else {
            temp = false;
        }
        await humanScroll(driver); // scroll humain après chaque chargement
    }
    ...
    for (let originePub in listmypub) {
        ...
        if (numberCite0.length > 0) {
            ...
            updatedList.push({ title: title1, citingLink: numberCite1.length > 0 ? await numberCite.getAttribute('href') : '', citenumber: numberCite1.length > 0 ? numberCite1 : 0 });
        }
    }

    // --------------------------------------------------------------------
    // PARCOURIR LES CITATIONS (TOUTES LES PAGES)
    // --------------------------------------------------------------------
    for (let pubIt in updatedList) {
        ...
        if ((targetNb > -1) && (res[targetNb].citenumber == updatedList[pubIt].citenumber)) continue;
        ...
        if (updatedList[pubIt].citenumber > 0) {
            ...
            while (whiletest) {
                ...
                for (let pub in listpub) {
                    ...
                    for (let pubb in auths) {
                        if (pub6.length > counter && await pub6[counter].getText() === auths[pubb]) {
                            let authlink0 = await pub6[counter].getAttribute('href');
            ...
            publi = { title: updatedList[pubIt].title, citingLink: '', citenumber: 0, citing: [] };
        }
