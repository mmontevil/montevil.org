...
async function login(driver) {
  ...
  await sleep(1000);
  await driver.findElement(By.xpath("//button[@mode='secondary']")).click();
  ...
  try {
    ...
    for (iter in titles) {
      try {
        ...
        await driver.wait(() => documentInitialised(), 10000);
        await sleep(2000);
        let checkrec = await driver.findElement(
          By.xpath("//div[@class='content-layout']/div[3]/div[1]/div[1]")
        );
        ...
        for (entry in link) {
          ...
          for (const entry in bib) {
            if (
              search(slugifyString(bib[entry].title.toLowerCase()), title0, 4)
                .length > 0
            ) {
          ...
          let mentioned =
            'https://montevil.org/publications/varia/' + idd + '/';
          if (typepub == 'article-journal')
            mentioned =
              'https://montevil.org/publications/articles/' + idd + '/';
          if (typepub == 'chapter')
            mentioned =
              'https://montevil.org/publications/chapters/' + idd + '/';
          if (typepub == 'book')
