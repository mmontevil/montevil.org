...
var findSection = function (centeredElement) {
  if (
    centeredElement.classList.contains('legibilityWidth') ||
    centeredElement.classList.contains('wrap0')
  ) {
    return null;
  } else {
    ...
    while (element) {
      if (
        element.matches('.legibilityWidth > *') ||
        element.matches('.wrap0 > *')
      )
        return getPreviousSibling(element, 'h2,h3,h4,h5,h6');
...
function openNav() {
  if (myNav.style.height === '100%') {
    ...
    navToc.setAttribute("aria-expanded", "false");
   ...
   // navToc.classList.remove('onclickbuttonon');
    ...
    var allButtons = document.querySelectorAll(".navButton");
 ...
 //   navToc.classList.add('onclickbuttonon');
        navToc.setAttribute("aria-expanded", "true");
    ...
    if (sectionId && sectionId != '') {
      tocSelectedEle = document.querySelectorAll(
        '#myNav a[href^="#' + sectionId + '"]'
      )[0];
      if (tocSelectedEle) {
...
window.toggleSubNav=function(eID) {
...
const loadFooter = () => {
  ...
  if (widthem > 40)
    backgroundImageWidth = Math.ceil((window.viewport_width * 1.7) / 20) * 20;
  if (widthem > 80)
...
annytab.lightbox = (function () {
  ...
  function lightbox(opts) {
    ...
    // Set options
    this.options = {
      selector: '.annytab-lightbox-popup',
      fade_duration: 1000,
  ...
  function addLinkEvents(lb, links) {
    ...
    for (var i = 0; i < links.length; i++) {
      // Add a click event
      window.onload = links[i].addEventListener(
        'click',
        function (event) {
  ...
  function addContainerEvents(lb) {
    // Add a close event
    window.onload = lb.close_button.addEventListener(
      'click',
      function (event) {
        // Prevent default click behaviour
        event.preventDefault();
    ...
    window.onload = lb.zoom_button.addEventListener(
      'click',
      function (event) {
        // Prevent default click behaviour
        event.preventDefault();
    ...
    if (lb.options.close_click_outside === true) {
      window.onload = lb.container.addEventListener(
        'click',
        function (event) {
          // Prevent default click behaviour
    ...
    if (lb.last_slide > 0) {
      ...
      window.onload = lb.left_arrow.addEventListener(
        'click',
        function (event) {
          // Prevent default click behaviour
          event.preventDefault();
      ...
      window.onload = lb.right_arrow.addEventListener(
        'click',
        function (event) {
          // Prevent default click behaviour
          event.preventDefault();
      ...
      if (lb.options.slideshow === true) {
        lb.automatic_slideshow = setInterval(function () {
          lb.showSlide(1);
        }, lb.options.slideshow_interval);
      }
    } else {
      // Hide arrows
  ...
  lightbox.prototype.showSlide = function (step) {
    ...
    var next_slide = this.container.querySelector(
      'img[data-lightbox-id="' + this.current_slide + '"]'
    );
    // Set a caption
  ...
  lightbox.prototype.open = function (link) {
    ...
    // Add the first image
    var source =
      '<img id="lighbox-img" data-lightbox-id="0" src="' +
      href +
      '" class="annytab-lightbox-image ' +
      classes +
    ...
    for (var i = 0; i < images.length; i++) {
      ...
      if (url !== href) {
        source +=
          ...
          '" alt="image" style="display:none;"';
        source +=
          images[i].getAttribute('data-lightbox-caption') !== null
            ? ' data-lightbox-caption="' +
              images[i].getAttribute('data-lightbox-caption')
            : '';
        source += '" />';
        counter += 1;
      }
    ...
    this.container.insertAdjacentHTML(
      'beforeend',
      '<div class="annytab-lightbox-margin">' +
    ...
    this.caption_container = this.container.querySelector(
      '.annytab-lightbox-caption'
    );
    this.left_arrow = this.container.querySelector(
      '.annytab-lightbox-left-arrow'
    );
    this.right_arrow = this.container.querySelector(
    ...
    // Show the next slide
    this.showSlide(1); /*,   // Parent Container Element
  ...
  lightbox.prototype.close = function () {
    ...
    if (this.touchControl) this.touchControl.removePanEvents();
  ...
  lightbox.prototype.zoom = function () {
    if (this.zoomed) {
      ...
      this.image0.setAttribute(
        'style',
        'max-width: max-content;max-height: max-content'
      );
      this.touchControl = new ImageMove('lighbox-cont', 'lighbox-img');
      this.touchControl.addPanEvents();
    }
...
var lightbox = new annytab.lightbox({
  selector: '.glightbox',
  fade_duration: 300,
  close_click_outside: true,
  slideshow: true,
...
(function (win, doc, DomFunctionalityClass) {
  var domFunctionality = new DomFunctionalityClass(doc),
    ...
    addCustomEvents = function () {
      domFunctionality.onready(function () {
        ...
        domFunctionality.addEventFromElement(
          ImageContainerElement,
          customEventObj
        );
      }, this);
    ...
    removeCustomEvents = function () {
      var customEventObj = EVENT_MAP[this.customeventName];
      domFunctionality.removeEventFromElement(
        ImageContainerElement,
        customEventObj
      );
    },
    touchStart = function (event) {
      ...
      event.clientX
        ...
        : ((xpos = event.touches[0].pageX), (ypos = event.touches[0].pageY));
      DRAGGGING_STARTED = 1;
      LAST_MOUSE_POSITION = {
        x: xpos - DIV_OFFSET.left,
        y: ypos - DIV_OFFSET.top,
      };
    },
    ...
    touchMove = function (event) {
      ...
      if (DRAGGGING_STARTED == 1) {
        event.clientX
          ? ((curr_x_pos = event['clientX']), (curr_y_pos = event['clientY']))
          : ((curr_x_pos = event.touches[0].pageX),
            (curr_y_pos = event.touches[0].pageY));
        current_mouse_position = {
          x: curr_x_pos - DIV_OFFSET.left,
          y: curr_y_pos - DIV_OFFSET.top,
        };
        ...
        img_top_new < CONTAINER_HEIGHT - IMAGE_HEIGHT &&
          (img_top_new = CONTAINER_HEIGHT - IMAGE_HEIGHT);
        img_left_new > 0 && (img_left_new = 0);
        img_left_new < CONTAINER_WIDTH - IMAGE_WIDTH &&
          (img_left_new = CONTAINER_WIDTH - IMAGE_WIDTH);
        imgElement.getSetCss({
          top: img_top_new + 'px',
          left: img_left_new + 'px',
        });
        (event.clientX > CONTAINER_WIDTH || event.clientY > CONTAINER_HEIGHT) &&
          (DRAGGGING_STARTED = 0);
      }
    },
    EVENT_MAP = {
  ...
  typeof win.addEventListener === 'undefined' &&
    (function () {
      var registry = [],
        ...
        removeEvent = function (type, listner) {
          for (var index = 0; index < registry.length; index++) {
            if (
              ...
              registry[index].listner === listner
            ) {
          ...
          if (typeof value === 'object') {
            ...
            cssValue =
              typeof getComputedStyle !== 'undefined'
                ...
                : element.currentStyle[value];
            !isNaN(parseInt(cssValue))
              ? (cssValue = cssValue)
              : (cssValue = '0px');
            return cssValue;
          }
